...
pensadas para a realidade do qobuz-dl-ultra:

  * **Match por ID de tag primeiro.** Arquivos baixados por este projeto trazem
    ``QOBUZALBUMID``/``BARCODE``. Se a tag existe, o match é *certo* (não
    depende de nome). O libsync só tinha match por nome.
  * **Match por UPC** (``BARCODE``) contra o catálogo.
  * **Fuzzy de verdade** (via ``qobuz_dl.fuzzy``, que usa rapidfuzz se houver e
...
class FolderMeta:
    ...
    service_album_id: Optional[str] = None  # QOBUZALBUMID / TIDALALBUMID
    ...
    try:
        ...
        if ext == ".flac":
            ...
            out["service_album_id"] = _first(
                f.get("QDL_ALBUM_ID") or f.get("QOBUZALBUMID") or f.get("TIDALALBUMID")
            )
            ...
            t = ID3(str(path))
            out["service_album_id"] = _first(
                t.get("TXXX:QDL_ALBUM_ID")
                or t.get("TXXX:QOBUZALBUMID")
                or t.get("TXXX:TIDALALBUMID")
            )
