...
for filepath in glob.glob('qobuz_dl/**/*.py', recursive=True):
    ...
    tc = re.sub(r'([fF]?\"\"\"[\s\S]*?\"\"\"|[fF]?\'\'\'[\s\S]*?\'\'\')', save_triple, content)
    
    # 2. Lógica cirúrgica: só tira o enter de dentro das chaves { }
    def fix_fstring(m):
        s = m.group(0)
        res = []
        in_brace = 0
        for c in s:
            if c == '{': 
                ...
                res.append(' ') # Troca o Enter por espaço
            else: 
                res.append(c)
        return ''.join(res)
    
    # Busca apenas f-strings normais (f\"...\" ou f\'...\')
    tc = re.sub(r'[fF]\"(?:[^\"\\]|\\[\s\S])*\"|[fF]\'(?:[^\'\\]|\\[\s\S])*\'', fix_fstring, tc)
