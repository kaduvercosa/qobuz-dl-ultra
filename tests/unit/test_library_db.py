...
def test_upsert_nao_apaga_campos_com_none(lib):
    aid = lib.upsert_album("qobuz", "1", "T", "A", cover_url="http://c", bit_depth=24)
    lib.upsert_album("qobuz", "1", "T2", "A", cover_url=None, bit_depth=None)
...
def test_upsert_preserva_estado_de_download(lib):
    aid = lib.upsert_album("qobuz", "1", "T", "A")
...
def test_mark_removed_e_retorno(lib):
    lib.upsert_album("qobuz", "1", "T1", "A")
    lib.upsert_album("qobuz", "2", "T2", "A")
    removed = lib.mark_removed("qobuz", ["1"])
    assert [r["source_album_id"] for r in removed] == ["2"]
    assert lib.mark_removed("qobuz", ["1"]) == []  # idempotente
    lib.upsert_album("qobuz", "2", "T2", "A")  # voltou
    assert lib.get_album_by_source_id("qobuz", "2")["removed_from_service"] == 0


def test_reset_stuck_e_relatorios(lib):
    a = lib.upsert_album("qobuz", "1", "T1", "A")
    b = lib.upsert_album("qobuz", "2", "T2", "A")
...
def test_historico_de_sync(lib):
    r1 = lib.create_sync_run("qobuz")
    lib.complete_sync_run(r1, albums_found=3, albums_new=1)
    r2 = lib.create_sync_run("qobuz")
    lib.fail_sync_run(r2)
    r3 = lib.create_sync_run("qobuz")
    lib.interrupt_sync_run(r3)
    hist = lib.get_sync_history("qobuz")
...
def test_schema_reabre_sem_erro(tmp_path):
    LibraryDB(tmp_path / "l.db").upsert_album("qobuz", "1", "T", "A")
