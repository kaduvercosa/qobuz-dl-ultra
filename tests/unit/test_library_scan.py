...
def test_dry_run_nao_grava(tmp_path, lib):
    aid = lib.upsert_album("qobuz", "1", "Discovery", "Daft Punk")
...
def test_contagem_divergente_vai_para_revisao(tmp_path, lib):
    lib.upsert_album("qobuz", "1", "Discovery", "Daft Punk", track_count=14)
...
def test_dois_candidatos_nunca_auto(tmp_path, lib):
    lib.upsert_album("qobuz", "1", "Greatest Hits", "Queen")
    lib.upsert_album("qobuz", "2", "Greatest Hits (Deluxe)", "Queen")
...
def test_fuzzy_sugere_mas_nunca_marca(tmp_path, lib):
    lib.upsert_album("qobuz", "1", "The Wall", "Pink Floyd")
...
def test_adocao_de_album_fora_dos_favoritos(tmp_path, lib, monkeypatch):
    ...
    row = lib.get_album_by_source_id("qobuz", "zzz")
    ...
    assert rep2["unmatched"] and lib2.get_album_by_source_id("qobuz", "zzz") is None
...
def test_dedup_writer_e_falha_isolada(tmp_path, lib):
    lib.upsert_album("qobuz", "1", "A", "Art")
    lib.upsert_album("qobuz", "2", "B", "Art")
...
def test_reconcile_sentinelas(tmp_path, lib):
    root = tmp_path / "m"
    ok = _mk(root, "a", 2)
    sn.write_sentinel(ok, sn.build_payload("qobuz", "10", "T", "A", 2))
    ...
    assert lib.get_album_by_source_id("qobuz", "10")["download_status"] == "complete"
