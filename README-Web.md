# qobuz-dl-ultra (monorepo: CLI + Web)

> Este repositório agora combina dois projetos que antes viviam separados.
> O conteúdo original da CLI (`qobuz_dl/`, `tests/`, `tools/` — README completo
> logo abaixo) **não foi alterado nesta etapa**. O app web do `libsync`
> (FastAPI + SvelteKit) foi movido pra pasta [`web/`](web/), também **sem
> nenhuma alteração interna** — só o lugar dele mudou.

## Estrutura

| Pasta | O quê | Status |
|---|---|---|
| raiz (`qobuz_dl/`) | CLI -- Qobuz | ✅ Igual a sempre, não foi tocado |
| [`web/`](web/) | App web -- Qobuz + Tidal | ✅ Igual a sempre, não foi tocado |
| raiz (`qobuz_dl/`) | CLI -- Tidal | 🔜 Ainda não existe (próxima etapa) |

Todo comando do `web/` continua igual ao projeto original, só precisa rodar
de dentro da pasta `web/`:
```bash
cd web
git submodule update --init --recursive   # ver aviso abaixo
make deps
make dev
```

## ⚠️ Sobre o submódulo do SDK (`web/sdks/qobuz_api_client`)

O app web depende de um SDK externo (`arthursoares/qobuz_tidal_api_client`)
consumido como *git submodule*. Um `.zip` não carrega submódulos git -- essa
pasta **vai estar vazia** até você inicializar o submódulo de verdade dentro
de um clone git real (o `.gitmodules` na raiz deste repositório já aponta
pro caminho novo, `web/sdks/qobuz_api_client`):
```bash
git submodule update --init --recursive
```

## Por que dois `pyproject.toml` separados, e não um só

A CLI usa `setuptools`/`hatch` e o Web usa `poetry` -- gerenciadores de
dependência diferentes. Fundir os dois arriscava quebrar a instalação de um
por causa do outro. Manter cada um isolado na sua pasta é a forma mais
segura de garantir que nada que já funcionava parasse de funcionar só pela
junção.

---

# 𝗤𝗢𝗕𝗨𝗭-𝗗𝗟 𝗨𝗟𝗧𝗥𝗔 (README original da CLI)
...
# para salvar tokens sem criptografia no config.ini em vez do Keyring do SO.
...
> **🔒 Nota de Segurança:** Com o Keyring ativo, `auth_token` e `genius_token` são migrados para o gerenciador de credenciais do sistema. Com `disable_keyring = true`, eles permanecem sem criptografia no `config.ini`; a permissão `0600` restringe quem pode abrir o arquivo, mas não criptografa seu conteúdo.
...
Selecione **`yes`** se estiver em um ambiente de servidor ou NAS. Isso ignora o chaveiro do sistema e persiste tokens sem criptografia no `config.ini`. A permissão `0600` restringe o acesso ao usuário atual, mas não criptografa o conteúdo; proteja e nunca compartilhe esse arquivo.
...
É interativo: pergunta a pasta de música (sugerindo a do `config.ini` ou o diretório atual), varre as subpastas em busca de arquivos de áudio, e mostra um menu pra você escolher qual inspecionar. Reusa a mesma lógica de `qobuz_dl/utils.py` usada pelo downloader via FFmpeg.
